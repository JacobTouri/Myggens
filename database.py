...
def _resolve_db_path() -> str:
    """Find DB-path på en måde der fungerer både lokalt og i produktion.

    Prioritet:
      1) DB_PATH env var (bruges i deployment, fx /data/database.sqlite3)
      2) ./data/database.sqlite3 (din oprindelige struktur)
      3) ./database.sqlite3 (fallback, hvis db ligger ved siden af filerne)
    """
    ...
    # Default til den “rigtige” mappe, selv hvis filen ikke findes endnu
    return candidates[0]
...
# Sørg for at folderen findes (fx /data på en mounted volume)
